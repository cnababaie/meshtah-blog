...
function slugify(text) {
  return text
    ...
    .replace(/[^\u0600-\u06FF\w\-]+/g, '') // keep Persian + word chars
    .toLowerCase();
}
...
export default async function Page({ params }) {
  ...
  const headings = extractHeadings(poem.text);

  // add main H1 from article.name at the start
  if (poem.name) {
    headings.unshift({ text: poem.name, id: slugify(poem.name), level: 1 });
  }
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 md:mx-16 md:my-8">
      <article className="flex-1 bg-white md:rounded-lg shadow-md overflow-hidden">
        <div className="headerimg">
          <img
            className="md:rounded-lg"
            src={`http://localhost:1337${poem.image.url}`}
            alt={poem.name}
          />
        </div>

        <div className="content md:px-8 px-4 singlepoem">
          <div className="top mb-8 text-white md:p-8 py-4">
            <h1 id={slugify(poem.name)} className="text-3xl pb-4 px-8 font-bold">
              {poem.name}
            </h1>
      ...
      <aside className="w-full md:w-1/3 sticky top-[6rem] self-start">
        <div className="bg-gray-100 md:rounded-lg shadow-md p-4 mb-2">
          <h2 className="font-bold mb-4">اطلاعات شعر</h2>
          <hr className="my-4" />
          <ul className="space-y-2">
            <li>
              <strong>نویسنده:</strong> {(poem.writers ?? []).map(w => w.name).join(", ")}
            </li>
            <li>
              <strong>مترجم:</strong> {(poem.translators ?? []).map(w => w.name).join(", ")}
            </li>
            <li>
              <strong>سال چاپ:</strong>{' '}
              {new Intl.NumberFormat('fa-IR', { useGrouping: false }).format(
                poem.OriginalPublishedYear
              )}
            </li>
            <li>
        ...
        <div className="bg-gray-100 md:rounded-lg shadow-md p-4 mb-2">
          <h2 className="font-bold mb-4">فهرست</h2>
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      title: poem.name,
      description: poem.description,
      images: [`http://localhost:1337${poem.image?.url}`],
    },
  };
}
