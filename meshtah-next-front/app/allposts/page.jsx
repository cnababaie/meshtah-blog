    ...
    return (
        <div>
            <div className="all-posts md:my-16 md:mx-64 p-8 bg-white/70 md:rounded-lg shadow-md">
                <h2 className="text-2xl font-bold mb-4">واردات</h2>
                ...
                <div className="grid grid-cols-1 gap-6 text-white">
                    {allPosts.map((post) => (
                        <div key={post.id} className="card border rounded-lg hover:shadow-2xl transition-shadow relative">
                            <Link href={post.url}>
                                <span className='pubdate absolute bg-dustyred z-10 p-1 px-2 rounded left-2 top-2'>
                                    {new Date(post.createdAt || post.attributes?.createdAt).toLocaleDateString('fa-IR', {year: 'numeric', month: 'long', day: 'numeric'})}
                                </span>
                                <span className='pubdate absolute bg-dustyred z-10 p-1 px-2 rounded left-32 top-2'>
                                      {post.type === "poem"
                                        ? "شعر"
                                        : "مقاله"}
                                </span>
                                ...
                                <img 
                                    ...
                                    className="inline-block md:max-w-[40%] md:h-[50%] object-cover"
                                />

                                <div className='hidden md:inline-block max-w-[60%] text-black align-top px-4 py-12 text-lg h-full bg-white'>
                                    <p>
                                        {post.description}
                                    </p>
